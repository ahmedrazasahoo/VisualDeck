<template>
  <div class="vertical-card circular-card" :style="{
    '--primary-color-rgba': primaryColorRgba,
    '--secondary-color-rgba': secondaryColorRgba
  }">
    <div class="circular-wrapper">
      <div class="circular-image-ring">
        <div class="circular-image-holder">
          <img :src="card.image" :alt="card.title" :class="['card-image', `image-shape-${imageShape}`]" />
        </div>
        <div v-if="showIdBadge" class="circular-badge">{{ card.id }}</div>
      </div>
      <div class="circular-info">
        <h3 class="circular-title">{{ card.title }}</h3>
        <div v-if="showCategory" class="circular-category">{{ card.category }}</div>
        <div v-if="showDescription" class="circular-description">
          <p>{{ card.description }}</p>
        </div>
        <div v-if="showPrice" class="circular-price">{{ card.price }}</div>
      </div>
    </div>
  </div>
</template>
...
.circular-badge {
  ...
  border: 3px solid v-bind(cardBackgroundColor);
}
...
.image-shape-rounded { border-radius: 50%; }
.image-shape-square { border-radius: 0; clip-path: none; }
.image-shape-circle { clip-path: circle(50% at 50% 50%); }
