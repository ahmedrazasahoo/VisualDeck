<template>
  <div class="vertical-card luxury-card" :style="{
    '--primary-color-rgba': primaryColorRgba,
    '--secondary-color-rgba': secondaryColorRgba
  }">
    <!-- Section 1: Badge Block -->
    <div v-if="showIdBadge" class="luxury-badge-section">
      <div class="header-decorative-line"></div>
      <div class="luxury-id">
        <span class="id-label">№</span>
        <span class="id-value">{{ String(card.id).padStart(3, '0') }}</span>
      </div>
      <div class="header-decorative-line"></div>
    </div>
    ...
    <div class="luxury-title-section">
      <div v-if="showCategory" class="luxury-category">{{ card.category }}</div>
      <h3 class="luxury-title">{{ card.title }}</h3>
    </div>

    <!-- Section 4: Description Block -->
    <div v-if="showDescription" class="luxury-description-section">
      <p class="luxury-description">{{ card.description }}</p>
    </div>

    <!-- Section 5: Price Block -->
    <div v-if="showPrice" class="luxury-price-section">
      <div class="price-divider"></div>
      <span class="luxury-price">{{ card.price }}</span>
      <div class="price-divider"></div>
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  cardBackgroundColor: { type: String, default: '#1a1a1a' },
  cardTitleColor: { type: String, default: '#f5f5f5' },
  cardDescriptionColor: { type: String, default: 'rgba(255, 255, 255, 0.7)' },
  cardPriceColor: { type: String, default: '#d4af37' },
  cardCategoryBgColor: { type: String, default: '#d4af37' },
  cardBadgeColor: { type: String, default: '#d4af37' }
});
...
.header-decorative-line {
  flex: 1;
  height: 1px;
  background: linear-gradient(90deg, transparent 0%, v-bind(cardBadgeColor) 50%, transparent 100%);
}
...
.image-border-frame {
  ...
  height: 240px;
  padding: 0.4rem;
  background: linear-gradient(135deg, v-bind(cardBadgeColor) 0%, v-bind(cardPriceColor) 100%);
...
.price-divider {
  flex: 1;
  height: 1px;
  background: linear-gradient(90deg, transparent 0%, v-bind(cardPriceColor) 50%, transparent 100%);
}

.luxury-price {
  font-size: var(--price-size, 1.25rem);
  font-weight: 700;
