<template>
  <div class="vertical-card metro-card" :style="{
    '--primary-color-rgba': primaryColorRgba,
    '--secondary-color-rgba': secondaryColorRgba
  }">
    <div class="metro-image-wrapper">
      <img :src="card.image" :alt="card.title" :class="['card-image', `image-shape-${imageShape}`]" />
      <div v-if="showIdBadge" class="metro-badge">{{ card.id }}</div>
    </div>
    <div class="metro-content">
      <div class="metro-header">
        <h3 class="metro-title">{{ card.title }}</h3>
        <div v-if="showCategory" class="metro-category">{{ card.category }}</div>
      </div>
      <div v-if="showDescription" class="metro-description">
        <p>{{ card.description }}</p>
      </div>
      <div v-if="showPrice" class="metro-footer">
        <div class="metro-price">{{ card.price }}</div>
      </div>
    </div>
  </div>
</template>
