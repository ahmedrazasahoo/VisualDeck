<template>
  <div class="vertical-card magazine-card" :style="{
    ...
    <div class="magazine-hero">
      ...
      <div class="magazine-overlay">
        <div v-if="showIdBadge" class="magazine-issue">ISSUE {{ String(card.id).padStart(2, '0') }}</div>
        <h3 class="magazine-headline">{{ card.title }}</h3>
      </div>
    </div>
    <div class="magazine-article">
      <div v-if="showCategory" class="magazine-section">{{ card.category }}</div>
      <div v-if="showDescription" class="magazine-excerpt">
        <p>{{ card.description }}</p>
      </div>
      <div v-if="showPrice" class="magazine-details">
        <span class="magazine-price">{{ card.price }}</span>
      </div>
    </div>
  </div>
</template>
...
.magazine-section {
  ...
  border-bottom: 2px solid v-bind(cardCategoryBgColor);
}
