<template>
  <div class="vertical-card gradient-card" :style="{
    '--primary-color-rgba': primaryColorRgba,
    '--secondary-color-rgba': secondaryColorRgba
  }">
    <!-- Section 1: Badge Block -->
    <div v-if="showIdBadge" class="gradient-badge-section">
      <div class="gradient-badge">{{ card.id }}</div>
    </div>

    <!-- Section 2: Image Block -->
    <div class="gradient-image-section">
    ...
    <div class="gradient-title-section">
      ...
      <div v-if="showCategory" class="gradient-category">{{ card.category }}</div>
    </div>

    <!-- Section 4: Description Block -->
    <div v-if="showDescription" class="gradient-description-section">
      <p class="gradient-description">{{ card.description }}</p>
    </div>

    <!-- Section 5: Price Block -->
    <div v-if="showPrice" class="gradient-price-section">
      <span class="gradient-price">{{ card.price }}</span>
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  cardBadgeBgColor: { type: String, default: 'rgba(255, 255, 255, 0.2)' },
