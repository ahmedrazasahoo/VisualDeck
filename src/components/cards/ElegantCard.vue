<template>
  <div class="vertical-card elegant-card" :style="{
    '--primary-color-rgba': primaryColorRgba,
    '--secondary-color-rgba': secondaryColorRgba
  }">
    <!-- Section 1: Badge/Category Block -->
    <div class="elegant-header-section">
      <div v-if="showIdBadge" class="elegant-id">No. {{ card.id }}</div>
      <span v-if="showCategory" class="elegant-category">{{ card.category }}</span>
    </div>

    <!-- Section 2: Image Block -->
    <div class="elegant-image-section">
    ...
    <div class="elegant-title-section">
      <div class="title-divider"></div>
      <h3 class="elegant-title">{{ card.title }}</h3>
      <div class="title-divider"></div>
    </div>

    <!-- Section 4: Description Block -->
    <div v-if="showDescription" class="elegant-description-section">
      <p class="elegant-description">{{ card.description }}</p>
    </div>

    <!-- Section 5: Price Block -->
    <div v-if="showPrice" class="elegant-price-section">
      <span class="price-label">Starting at</span>
      <span class="elegant-price">{{ card.price }}</span>
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  cardBackgroundColor: { type: String, default: '#fafafa' },
  ...
  cardBadgeColor: { type: String, default: '#999999' }
});
...
.vertical-card {
  ...
  border-image: linear-gradient(90deg, var(--primary-color-rgba) 0%, var(--secondary-color-rgba) 100%) 1;
}
...
.title-divider {
  flex: 1;
  height: 2px;
  background: linear-gradient(90deg, transparent, var(--primary-color-rgba), transparent);
}
...
.elegant-price {
  font-size: var(--price-size, 1.35rem);
  font-weight: 700;
