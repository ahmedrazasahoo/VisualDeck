<template>
  <div class="vertical-card bold-card" :style="{
    '--primary-color-rgba': primaryColorRgba,
    '--secondary-color-rgba': secondaryColorRgba
  }">
    <!-- Section 1: Badge/ID Block -->
    <div v-if="showIdBadge" class="bold-badge-section">
      <div class="badge-id">#{{ card.id }}</div>
    </div>

    <!-- Section 2: Image Block -->
    <div class="bold-image-section">
    ...
    <div v-if="showDescription" class="bold-description-section">
      <p class="bold-description">{{ card.description }}</p>
    </div>

    <!-- Section 5: Price Block -->
    <div v-if="showPrice" class="bold-price-section">
      <span class="bold-price">{{ card.price }}</span>
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  cardFooterBgColor: { type: String, default: '#000000' }
});
