<template>
  <div class="vertical-card wave-card" :style="{
    '--primary-color-rgba': primaryColorRgba,
    '--secondary-color-rgba': secondaryColorRgba
  }">
    <div class="wave-section header-section">
    ...
    <div class="wave-section content-section">
      <h3 class="wave-title">{{ card.title }}</h3>
      <div v-if="showCategory" class="wave-category">{{ card.category }}</div>
    </div>

    <div v-if="showDescription" class="wave-section description-section">
      <p class="wave-description">{{ card.description }}</p>
    </div>

    <div v-if="showPrice" class="wave-section price-section">
...
.description-section {
  flex: 1;
  padding: 1rem var(--content-padding, 16px) 1.5rem;
  background: v-bind(cardBackgroundColor);
  display: flex;
  align-items: center;
  justify-content: center;
