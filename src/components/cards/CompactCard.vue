<template>
  <div class="vertical-card compact-card" :style="{
    '--primary-color-rgba': primaryColorRgba,
    '--secondary-color-rgba': secondaryColorRgba
  }">
    <!-- Section 1: Badge/ID Block -->
    <div v-if="showIdBadge" class="compact-badge-section">
      <div class="compact-badge">{{ card.id }}</div>
    </div>
    ...
    <div class="compact-title-section">
      <div v-if="showCategory" class="compact-category">{{ card.category }}</div>
      <h3 class="compact-title">{{ card.title }}</h3>
    </div>

    <!-- Section 4: Description Block -->
    <div v-if="showDescription" class="compact-description-section">
      <p class="compact-description">{{ card.description }}</p>
    </div>

    <!-- Section 5: Price Block -->
    <div v-if="showPrice" class="compact-price-section">
      <span class="compact-price">{{ card.price }}</span>
    </div>
  </div>
</template>
...
.compact-title-section {
  ...
  flex-direction: column;
  gap: 0.5rem;
  background: linear-gradient(to bottom, #fafafa 0%, v-bind(cardBackgroundColor) 100%);
}
