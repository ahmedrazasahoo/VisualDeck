<template>
  <div class="vertical-card vibrant-card" :style="{
    '--primary-color-rgba': primaryColorRgba,
    '--secondary-color-rgba': secondaryColorRgba
  }">
    <!-- Section 1: Badge Block -->
    <div v-if="showIdBadge" class="vibrant-badge-section">
      <div class="vibrant-badge">#{{ card.id }}</div>
    </div>

    <!-- Section 2: Image Block -->
    <div class="vibrant-image-section">
      <img :src="card.image" :alt="card.title" :class="['card-image', `image-shape-${imageShape}`]" />
    </div>

    <!-- Section 3: Category/Title Block -->
    <div class="vibrant-title-section">
      <div v-if="showCategory" class="vibrant-category">{{ card.category }}</div>
      <h3 class="vibrant-title">{{ card.title }}</h3>
    </div>

    <!-- Section 4: Description Block -->
    <div v-if="showDescription" class="vibrant-description-section">
      <p class="vibrant-description">{{ card.description }}</p>
    </div>

    <!-- Section 5: Price Block -->
    <div v-if="showPrice" class="vibrant-price-section">
      <span class="vibrant-price">{{ card.price }}</span>
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  cardBadgeBgColor: { type: String, default: '#667eea' },
...
.vibrant-title-section {
  ...
  background: linear-gradient(to bottom, v-bind(cardBackgroundColor) 0%, #fafafa 100%);
  display: flex;
  flex-direction: column;
  gap: 0.75rem;
}
